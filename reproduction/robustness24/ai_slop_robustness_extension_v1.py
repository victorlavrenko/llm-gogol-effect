...
CODE_VERSION="1.1-extension"
SCHEMA_VERSION="1.0"
PROMPT_BANK_VERSION="ai-slop-robustness-independent-extension-v1"
PROMPT_BANK_EXPECTED_SHA256="00f76758a881710038e5a80188668fc8df9fc02737482d6c1fe843189b0aa22a"
...
def load_bank(path):
    ...
    if len(out)!=24: raise ValueError(f"expected 24 prompts, got {len(out)}")
...
class Client:
    ...
    def call(self,route,msgs,max_tokens):
        ...
        for attempt in range(1,self.retries+1):
            req=urllib.request.Request("https://openrouter.ai/api/v1/chat/completions",data=body,method="POST",headers={"Authorization":f"Bearer {self.key}","Content-Type":"application/json","HTTP-Referer":"https://example.invalid/anonymous-review","X-Title":"AI Slop Elicitation Robustness Extension"})
...
def manifest(out,args,qs,banksha,scriptsha):
    p={"code_version":CODE_VERSION,"schema_version":SCHEMA_VERSION,"script_filename":Path(__file__).name,"script_sha256":scriptsha,"prompt_bank_version":PROMPT_BANK_VERSION,"prompt_bank_sha256":banksha,"seed":args.seed,"models":MODEL_SPECS,"variants":{k:{"description":v.description,"scoring_instruction":v.scoring_instruction,"reverse_coded":v.reverse_coded} for k,v in VARIANTS.items()},"replay_question_ids":list(REPLAY_IDS),"n_prompts":len(qs),"temperature":args.temperature,"primary_robustness_claims":["On this fresh 24-prompt extension, mean normalized Post-minus-Live is positive for both models under each of the four elicitation variants.","Averaged over variants and matched prompts, Gemini has a larger normalized Post-minus-Live shift than GPT-5.5."],"secondary_descriptive_metrics":["Within-output Live-to-Post rank stability is reported descriptively and is not a success criterion for the extension."],"questions":[{"question_id":q.question_id,"family":q.family,"task":q.task} for q in qs]}
...
def main():
    ...
    frozen=args.out/"robustness_extension_prompt_bank_v1_frozen.jsonl";raw=args.prompt_bank.read_bytes()
    ...
    print(f"Runner: {sp.name} | SHA256={ssha}\nPrompt bank SHA256={bsha}\nManifest: {mp}\nPrompts={len(qs)} Models={len(MODEL_SPECS)} Variants={len(VARIANTS)} Live cells={len(qs)*len(MODEL_SPECS)*len(VARIANTS)} Post cells={len(qs)*len(MODEL_SPECS)*len(VARIANTS)} Replay output-cells=0")
