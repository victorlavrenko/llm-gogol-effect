...
\author{Victor Lavrenko\\[-1pt]\small PeaceTech VC, Israel\\[-1pt]\small \texttt{victor@peacetech.vc}}
\date{}
...
\section{Operationalizing IKEA-Like and Gogol Effects}
\subsection{Three Evaluation States and Two Candidate Effects}
...
We use two signed contrasts throughout. Because lower scores are more favorable to the generated text, an \emph{IKEA-like shift} is $R-L>0$: the model scores its output more favorably while it is still on the writing trajectory than when the same visible prefix is replayed in a fresh context. The \emph{Gogol effect} is $P-R>0$: the model scores that earlier text less favorably after the completed output becomes available. These are operational definitions tied to the present elicitation; neither is asserted to be a psychological mechanism.
...
\subsection{What the Framework Does and Does Not Identify}
...
\subsection{Task Probe and Frozen Prompt Sets}
...
\subsection{Live, Replay, and Post}
...
\subsection{Cross-Model Replay Breadth Extension}
...
\subsection{Pre-Specified Adaptive Confirmation}
...
\subsection{Held-Out Robustness Studies}
...
\subsection{Equal-Compute Rewrite Check}
...
\section{Results}
\subsection{Live and Retrospective Self-Evaluation Can Differ Substantially}
...
\subsection{IKEA-Like Self-Favoring Is Uncommon in the Six-Model Decomposition}
...
\subsection{The Gogol Effect Is More Frequent and Often Much Larger}
...
\begin{figure}[t]
\centering
\includegraphics[width=.98\linewidth]{six_model_effects.pdf}
\caption{Mean IKEA-like ($R-L$) and Gogol ($P-R$) shifts across six models. Error bars are descriptive 95\% $t$ intervals over prompts. GPT-5.5 and Gemini use fixed $N=40$ samples; the other four use adaptive breadth-extension sample sizes, so their intervals should not be interpreted as fixed-design confidence statements.}
\label{fig:six-model-effects}
\end{figure}

The original controls continue to support the interpretation in the two high-coverage confirmation models. In the 16-prompt Replay robustness cohort, Post--Replay exceeds Replay--Live in all eight model--elicitation combinations. In the separate $T=0$ control, Replay--Live is $-1.97$ and $-0.01$, while Post--Replay is $+7.13$ and $+25.27$ for GPT-5.5 and Gemini. Thus the post-completion shift in those models does not require stochastic sampling of the numeric score token at $T=0.7$.

\subsection{Retrospective Level Shifts and Rank Reliability Are Different Problems}
...
\begin{figure}[t]
\centering
\includegraphics[width=.98\linewidth]{reliability_map.pdf}
...
\subsection{Held-Out Elicitation Preserves the Direction of the Retrospective Shift}
...
\subsection{A Downstream Check: Good Target Ranking Is Not Sufficient for One-Shot Rewriting}
...
\section{Discussion}
\subsection{The Main Finding: IKEA-Like Bias Is Rare; the Gogol Effect Is More Frequent}
...
\subsection{What the Gogol Effect Does and Does Not Mean}
...
\subsection{Relation to Existing Trajectory-Confidence Work}
...
\subsection{Implications for Self-Evaluation and Inference-Time Control}
...
\section{Full Multi-Model Reliability Results}
...
\section{Grouped Calibration Results}
...
\section{Expanded Replay Statistics}
\label{app:replay}
...
\section{Correction Test Details}
...
\section{Held-Out Elicitation Robustness Details}
...
\section{Held-Out Temperature Control}
...
\section{Why Within-Output Ranking Is the Primary Local Metric}
\label{app:ranking}
